...
ability. **Code-mode / single-tool connectors:** some connectors funnel every operation through ONE catch-all tool (e.g. a code-mode MCP with a single `execute`/`invoke`) and expose NO separate read-vs-write tools, so the split is impossible there. Set the read-only discovery tools (search/get/list/schema) to Always allow, and set the catch-all to **Needs approval** (prompt on every call — the safe pick when customer writes are reachable) or Always allow (no prompt; rely on the read-only behavioral gate). Do NOT Block the catch-all — that disables the connector. VERIFIED 2026-08-05: an Auto-mode chat does NOT fire the per-tool "Needs approval" prompt (a read-only execute ran with no prompt in an Auto chat), and scheduled tasks run in Auto — so the connector write-prompt is a backstop ONLY in Manual-mode chats. For customer-write safety the PRIMARY control is the behavioral read-only gate (customer access is read-only); switch a chat to Manual for a hard prompt on a specific risky session; NEVER rely on the prompt in Auto / unattended contexts.
